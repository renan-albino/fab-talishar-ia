...
export const playCard = createAsyncThunk(
  ...
  async (params: { cardParams: Card; cardIndex?: number }, { getState }) => {
    ...
    const playNo =
      params.cardParams.actionDataOverride != null && params.cardParams.actionDataOverride !== ''
        ? params.cardParams.actionDataOverride
        : (params.cardIndex ?? 0);

    const cardAction = (params.cardParams as any).action ?? (params.cardParams as any).mode ?? 27;
    const mode = (cardAction !== undefined && cardAction !== 'undefined' && String(cardAction).trim() !== '') ? cardAction : 27;

    const queryParams = processInputParams(game.gameInfo);
    queryParams.set('mode', String(mode));
