...
|---|---|---|---|
| 1 | `chmod -R 777` é risco grave | ⚠️ Invalidação prática (necessário para Docker) | Inicialmente sugerido `775`, porém o container Apache (`www-data`, UID 33) e o host (`renan`, UID 1000) compartilham os volumes sem user namespace remapping. Com `775`, `www-data` perdeu permissão de escrita em `Talishar/HostFiles/GameIDCounter.txt` e `Talishar/Games/`, travando a criação de partidas no Talishar (`CreateGame.php`). Mantido `777` estritamente nas pastas de I/O de runtime (`Talishar/Games/`, `Talishar/HostFiles/`, `Talishar/AccountFiles/`, `Talishar/APIKeys/`, `logs/`). |
